 ...
 * priorityAP_* / nof_priorityAP_* / nof_priority_tempAP_* / legacy `averageAP`
 * lines first, then
 ...
 *   playerMin  = efficiency + minimumBonus + SKILL_BONUS * playerSkills
...
//   minimumBonus: 0 (normal) | 1 (priority) | 2 (priority + Neron's Only Friend)
const simulate = (kind, totalSkills, efficiency, minimumBonus = 0) => {
	const playerGain = (skills) => {
		const min = efficiency + minimumBonus + SKILL_BONUS * skills;
...
// field prefix → [gain kind, minimum bonus]
const VARIANTS = [
	['averageAP',      'average', 0],
	['nofAP',          'nof',     0],
	['priorityAP',     'average', 1],
	['nof_priorityAP', 'nof',     2],
];
...
const computeFields = (efficiency, type) => {
	...
	const fields = [];
	for (const [prefix, kind, minimumBonus] of VARIANTS) {
		if (!allowed.includes(prefix)) continue;
		for (let n = 0; n <= 4; n++) {
			fields.push([`${prefix}_${n}skill`, simulate(kind, n, efficiency, minimumBonus)]);
		}
	}
	return fields;
};

const AP_FIELD = /^\s*(?:average|nof|priority|nof_priority|nof_priority_temp)AP(?:_\dskill)?\s*:/;
