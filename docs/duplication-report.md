...
---
## C. Duplicated Calculation Logic
---

### C4. Empty result factories — similar-but-different structures

**Size: Medium** — 20–30 minutes, touches 3 files.

`FightCalculator._emptyResult()`, `EventDamageCalculator._emptyResult()` +
`_emptyDamageResult()`, and `ResourceCalculator._emptyResult()` each define
their own "return zero" structure. The damage sub-structure
`{ pessimist, average, optimist, worstCase, ...Prob }` is repeated across
`FightCalculator` and `EventDamageCalculator` with slight variations.

The reason it's medium and not small: the three empty results are structurally
*different* (fights have `fightingPower`/`grenadeCount`, events have
`tired`/`accident`/`disaster`, resources have 6 sub-objects). A shared factory
needs to either be generic or supply domain-specific fields on top of a common
base.

**Fix:** Add a `DamageDistributionEngine.emptyDamageResult()` for the shared
damage structure, and have each calculator compose it into its own
domain-specific empty result. Reduces the repeated damage scaffolding to one
place.


### C6. Damage scenario row-merging logic — copy-pasted

**Size: Medium** — 30–45 minutes, ~100 lines consolidated in one file.

`_renderCombatDamage()` (L200–L325) and `_renderEventDamage()` (L445–L545)
contain nearly identical ~100-line blocks of scenario collapsing:
- Is optimist === average? Merge label.
- Is average === pessimist? Merge label.
- Is worst === pessimist? Merge label.
- All equal? Show "All Scenarios".

The only difference between the two copies is the data source object
(`damage.*` vs `scenarios.*`).

**Fix:** Extract a `_buildDamageScenarioRows(data, hpIcon)` helper that takes
the damage/scenarios object and returns the HTML rows array. Both renderers
call it with their respective data. This also absorbs the `formatProb` fix
(C5) naturally.
