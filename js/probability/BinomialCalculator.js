...
const BinomialCalculator = {
	...
	/**
	 * Formats a distribution for console output.
	 * 
	 * @param {number} n - Number of trials
	 * @param {number} p - Probability
	 * @returns {string} Formatted string
	 */
	formatDistribution(n, p) {
		const dist = this.distribution(n, p);
		const lines = [`Binomial Distribution (n=${n}, p=${(p * 100).toFixed(1)}%):`];
		
		for (const entry of dist) {
			const bar = '█'.repeat(Math.round(entry.probability * 50));
			const probStr = (entry.probability * 100).toFixed(1).padStart(5);
			const cumStr = (entry.cumulative * 100).toFixed(1).padStart(5);
			lines.push(`  ${entry.k} events: ${probStr}% ${bar} (cum: ${cumStr}%)`);
		}

		const scenarios = this.scenarios(n, p);
		lines.push(`  ─────────────────────────────────`);
		lines.push(`  Optimist (25%):  ${scenarios.optimist} events`);
		lines.push(`  Average:         ${scenarios.average.toFixed(1)} events`);
		lines.push(`  Pessimist (75%): ${scenarios.pessimist} events`);
		lines.push(`  Worst Case:      ${scenarios.worstCase} events`);

		return lines.join('\n');
	},

	/**
	 * Generates HTML output for displaying distribution in the UI.
	 * 
	 * @param {number} n - Number of trials
	 * @param {number} p - Probability
	 ...
	 */
	generateHTML(n, p, eventName = 'Event') {
		const dist = this.distribution(n, p);
		const scenarios = this.scenarios(n, p);

		let html = `<div class="outcome-category">`;
		html += `<h5>${eventName} Distribution (${n} sectors, ${(p * 100).toFixed(1)}% each)</h5>`;

		// Distribution table
		html += `<div class="distribution-table">`;
		for (const entry of dist) {
			const widthPercent = Math.round(entry.probability * 100);
			html += `<div class="outcome-item">`;
			html += `<span class="outcome-label">${entry.k} occurrence${entry.k !== 1 ? 's' : ''}</span>`;
			html += `<span class="outcome-value neutral">${(entry.probability * 100).toFixed(1)}%</span>`;
			html += `</div>`;
		}
		html += `</div>`;

		// Scenarios
		html += `<div class="scenarios" style="margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.1);">`;
		html += `<div class="outcome-item"><span class="outcome-label">Optimist (25%)</span><span class="outcome-value positive">${scenarios.optimist}</span></div>`;
		html += `<div class="outcome-item"><span class="outcome-label">Average</span><span class="outcome-value neutral">${scenarios.average.toFixed(1)}</span></div>`;
		html += `<div class="outcome-item"><span class="outcome-label">Pessimist (75%)</span><span class="outcome-value warning">${scenarios.pessimist}</span></div>`;
		html += `<div class="outcome-item"><span class="outcome-label">Worst Case</span><span class="outcome-value danger">${scenarios.worstCase}</span></div>`;
		html += `</div>`;
	...
	/**
	 * Runs verification tests and outputs to console.
	 */
	runTests() {
		console.log('═══════════════════════════════════════════════════════');
		console.log('BinomialCalculator - Tests');
		console.log('═══════════════════════════════════════════════════════');

		// Test 1: Known values
		console.log('\n--- Test 1: Known Values ---');
		const p_5_2_03 = this.probability(5, 2, 0.3);
		console.log(`P(X=2) for n=5, p=0.3: ${p_5_2_03.toFixed(4)} (expected: 0.3087)`);
		console.log(Math.abs(p_5_2_03 - 0.3087) < 0.001 ? '✓ PASS' : '✗ FAIL');

		// Test 2: Distribution sums to 1
		console.log('\n--- Test 2: Distribution Sum ---');
		const dist = this.distribution(10, 0.4);
		const sum = dist.reduce((s, e) => s + e.probability, 0);
		console.log(`Sum of probabilities: ${sum.toFixed(6)} (expected: 1.000000)`);
		console.log(Math.abs(sum - 1) < 0.0001 ? '✓ PASS' : '✗ FAIL');

		// Test 3: Edge cases
		console.log('\n--- Test 3: Edge Cases ---');
		console.log(`P(k=0) when p=0: ${this.probability(5, 0, 0)} (expected: 1)`);
		console.log(`P(k=5) when p=1: ${this.probability(5, 5, 1)} (expected: 1)`);
		console.log(`P(k=3) when p=0: ${this.probability(5, 3, 0)} (expected: 0)`);

		// Test 4: Visual distribution
		console.log('\n--- Test 4: Distribution Visualization ---');
		console.log(this.formatDistribution(5, 0.3));
